...
static void update_menu_items(void) {
	...
	if(sys==SYSTEM1)
		MENU_ITEMS[0] = "正在管理：[系统1]";
	else if(sys==SYSTEM2)
		MENU_ITEMS[0] = "正在管理：[系统2]";
	else
		MENU_ITEMS[0] = "正在管理：[无效]";
}
...
void device_toggle_truedualboot(void) {
	...
	sprintf(confirm, "确定 - %s 双系统", enable?"关闭":"开启");

	if (confirm_selection("这将清空数据，确定吗？", confirm)) {
		// unmount /data
...
int device_get_truedualboot_entry(char* tdb_name) {
	sprintf(tdb_name, "双系统选项 [%s]", dualboot_is_tdb_enabled()?"已开启":"已关闭");
	return 0;
}
...
void device_choose_bootmode(void) {
	int sys = dualboot_select_system("默认启动：");
...
int device_get_bootmode(char* bootmode_name) {
	...
	sprintf(bootmode_name, "正在使用系统 %d", sysnum+1);
	return 0;
}
